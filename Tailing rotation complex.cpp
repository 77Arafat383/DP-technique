...
/*
Give a 3*N grid. and there are two types of tail 3*1 and 3*2. if rotation is allowed then 
how may way you can tail the grid. 
*/
...
ll f(int n, vector<int>&dp)
{
    ...
    return dp[n]=f(n-1,dp)+f(n-2,dp)+3LL*f(n-3,dp);
