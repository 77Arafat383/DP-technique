...
/*
Given an array and you have to find maximum sum of subsequences of this array. using dp. 
condition: can't take two consecutive numbers. 
*/

ll f(int i,vector<ll>&v , vector<ll>&dp )
{
    if(i==0)
    {
        if(v[i]>0) return v[i];
    ...
    ll ans=0;
    dp[i]=f(i-1,v,dp);
    if(v[i]>0) dp[i]=max(dp[i],f(i-2,v,dp)+v[i]);
    return dp[i];
