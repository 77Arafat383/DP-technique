...
/*
Given an array with N (N<=100000) integers. Find the subsequence of the array to find the following
expression A1-A2+A3-A4... 
*/

ll f(int i, int p, vector<ll>&v, vector<vector<ll>>&dp)
{
    if(i==si(v)) return 0;
    ...
    if(p&1)
    {
          dp[i][p]=max(dp[i][p],f(i+1,p^1,v,dp)+v[i]);
    }
    else
    {
        dp[i][p]=max(dp[i][p],f(i+1,p^1,v,dp)-v[i]);
